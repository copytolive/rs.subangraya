...
const pendanaan = [
  { utang: '40%', nilaiUtang: 'Rp344,4 M', modal: 'Rp516,7 M', dscr: '1,83x', moic: '4,59x', judul: 'Utang lebih rendah', warna: 'aman' },
  { utang: '50%', nilaiUtang: 'Rp430,5 M', modal: 'Rp430,5 M', dscr: '1,47x', moic: '5,46x', judul: 'Konservatif', warna: 'aman' },
  { utang: '60%', nilaiUtang: 'Rp516,7 M', modal: 'Rp344,4 M', dscr: '1,07x', moic: '7,08x', judul: 'Skenario bank dasar', warna: 'waspada' },
  { utang: '70%', nilaiUtang: 'Rp602,8 M', modal: 'Rp258,3 M', dscr: '0,90x', moic: '8,97x', judul: 'Utang lebih tinggi', warna: 'risiko' },
  { utang: '100%', nilaiUtang: 'Rp861,1 M', modal: 'Rp0', dscr: '0,63x', moic: '—', judul: 'Uji tekanan', warna: 'risiko' },
]
...
export default function Home() {
  const [pilihanPendanaan, setPilihanPendanaan] = useState(2)
  const [pilihanVisual, setPilihanVisual] = useState(0)
  const [tigaDimensi, setTigaDimensi] = useState(false)
  const skenario = pendanaan[pilihanPendanaan]
      ...
      <div className="halaman">
          ...
          <div className="hero-inner">
            ...
            <div className="hero-bawah">
              ...
              <div className="angka-utama">
                ...
                <div><b>Rp861,1 M</b><span>CAPEX DASAR · ANGGARAN INDIKATIF</span></div>
              </div>
            </div>
            <div className="catatan"><b>CATATAN KONTROL</b> Angka keuangan pada presentasi ini merupakan hasil perencanaan anggaran indikatif dan ilustratif, bukan CAPEX tersertifikasi, harga tender, komitmen pembiayaan, atau pembagian saham final. Tahap bukti eksternal T01–T11 masih terbuka.</div>
          </div>
        </header>
        ...
        <section id="pendanaan" className="bagian lembut">
          <div className="dalam pendanaan-layout">
            <div>
              <div className="judul-kecil">04 · Struktur pendanaan</div>
              <h2>Pilih porsi utang.<br/>Lihat <em>konsekuensinya.</em></h2>
              <p className="pengantar">Pilih skenario utang untuk melihat hubungan antara beban utang, kebutuhan modal sendiri, kemampuan membayar utang pada Tahun 4, dan kelipatan hasil investasi saat keluar.</p>
              <div className="pilihan">{pendanaan.map((item,index) => <button key={item.utang} className={pilihanPendanaan===index?'aktif':''} onClick={() => setPilihanPendanaan(index)}>{item.utang} UTANG</button>)}</div>
              <div className="komposisi"><div className="kolam" style={{'--isi':skenario.utang}}><b>{skenario.nilaiUtang}</b><span>PORSI UTANG</span></div><div className="plus">+</div><div className="kolam" style={{'--isi':`${100-parseInt(skenario.utang)}%`}}><b>{skenario.modal}</b><span>MODAL SENDIRI</span></div></div>
              <div className="mini">{pendanaan.map((item) => <div key={item.utang}><b>{item.dscr}</b><span>{item.utang} UTANG · DSCR TAHUN 4</span></div>)}</div>
            </div>
            <div className="skenario">
              <div className="skenario-head"><div><span>{skenario.judul.toUpperCase()} · SKENARIO TERPILIH</span></div><b>{skenario.utang}</b></div>
              <div className="skenario-grid"><div><span>NILAI UTANG</span><b>{skenario.nilaiUtang}</b></div><div><span>MODAL SENDIRI</span><b>{skenario.modal}</b></div><div><span>DSCR TAHUN 4</span><b className={skenario.warna}>{skenario.dscr}</b></div><div><span>MOIC SAAT KELUAR</span><b>{skenario.moic}</b></div></div>
              <div className="sinyal"><strong>Kesimpulan pendanaan saat ini:</strong> struktur dasar masih membutuhkan perlindungan melalui cadangan pembayaran bunga, masa tenggang lebih panjang, porsi utang lebih rendah, atau kombinasi sebelum pembiayaan dikunci. Seluruh skenario merupakan uji sensitivitas, bukan komitmen pemberi pinjaman.</div>
            </div>
          </div>
        </section>
